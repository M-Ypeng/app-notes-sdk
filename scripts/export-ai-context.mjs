...
const notesDir = path.join(projectRoot, '.app_notes');
...
async function readNotesFiles() {
  ...
  for (const file of files) {
    ...
    result.push({ fileName: file.name, data: JSON.parse(raw) });
  }
  return result;
}

function flattenNotes(files) {
  const notes = [];
...
function renderDetail(note, index) {
  ...
  const lines = [
    ...
    note.comment.images?.length ? `- 图片：${note.comment.images.map((item) => `\`${item}\``).join(', ')}` : undefined,
    note.fix?.summary ? `- 修复摘要：${note.fix.summary}` : undefined,
    note.fix?.changedFiles?.length ? `- 改动文件：${note.fix.changedFiles.map((item) => `\`${item}\``).join(', ')}` : undefined,
    ...
    ai.stepsToReproduce?.length ? `- 复现步骤：\n${ai.stepsToReproduce.map((step, stepIndex) => `  ${stepIndex + 1}. ${step}`).join('\n')}` : undefined,
    ai.fixHints?.length ? `- 修复线索：\n${ai.fixHints.map((hint) => `  - ${hint}`).join('\n')}` : undefined,
    ''
  ];
...
async function main() {
  const files = await readNotesFiles();
  ...
  const healthSummary = notes.reduce((acc, note) => {
  ...
  const content = [
    ...
    `项目：\`${path.relative(repoRoot, projectRoot) || '.'}\``,
    '',
    '## 总览',
    '',
    `- 备注文件：${files.length}`,
  ...
  console.log(`[app-notes] wrote ${path.relative(projectRoot, outputPath)}`);
}
