...
`page-path.ts` 负责页面路径获取、归一化和 `history.pushState` / `replaceState` 劫持。`app-notes-root` 在此基础上监听 `LOCATION_CHANGE_EVENT`、`popstate` 和 `hashchange`，从而覆盖常见 SPA programmatic 导航、浏览器前进后退和 hash 路由变化。
...
页面路径使用 `pathname + search + hash`，`normalizePagePath()` 会用浏览器 `URL` 解析为稳定的 pathname/search/hash 字符串，但不会额外排序 query string。
...
```ts
const cancel = scheduleAfterDomSettle(callback);
```

实现方式：立即执行一次，再通过双层 `requestAnimationFrame`、`100ms` 和 `320ms` 定时重试。返回值用于取消后续定时重试。
...
server 包同时提供面向宿主项目的导出命令：

```bash
app-notes-export-ai-context --root /path/to/project
```

参数：

- `--root` / `-r`：读取和写入 `.app_notes/` 的项目根目录，默认当前工作目录。
...
- `NotesStorage` 按 notes 文件名维护内存缓存。
- 缓存内容包含文件 `mtimeMs` 和 parsed JSON。
- 读取 notes 文件时，如果 mtime 未变化则直接使用缓存。
- 写入 notes 文件后同步更新对应缓存项。
...
- 列表接口可继续增加目录级缓存或按 pagePath 建索引，降低大量文件时的遍历成本。
...
宿主项目安装 server 包后，可使用 CLI：

```bash
app-notes-export-ai-context --root .
```
...
默认读取目标项目 `.app_notes/*.notes.json`，输出：
...
Markdown 导出面向 AI Coding 工具消费，应围绕以下字段组织内容。当前导出已覆盖 open 备注、页面、锚点、健康度、evidence、图片、视口、AI 补充字段和修复记录；后续增强 Export API 或 MCP Server 时，应尽量保持同一套字段语义。`schemaVersion` 保留在原始 `.notes.json` 中，AI 需要兼容判断时可按导出中的 `fileName` 回查原始 JSON。
